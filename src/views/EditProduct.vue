...
export default {
  ...
  methods: {
    ...
    editArray(editValue) {
      if (editValue.imageObj == "") {
        axios
          .put(
            `http://172.20.0.3:3000/images/updatename/${this.id}/${editValue.prodName}`
          )
          .then(() => {
        ...
        axios
          .put(
            `http://172.20.0.3:3000/images/update/${editValue.prodName}/${this.productByid.imageName}`,
            fd
          )
          .then(() => {
      ...
      axios
        .put(`http://172.20.0.3:3000/products/update/${this.id}`, editValue)
        .then((response) => {
    ...
    async fetchProductResult() {
      const res = await fetch(
        `http://172.20.0.3:3000/products/item/${this.id}`
      );
  ...
  async created() {
    ...
    this.imgURL = `http://172.20.0.3:3000/images/get/${this.productByid.imageName}`;
  },
};
</script>
