...
export default {
  ...
  methods: {
    ...
    editArray(editValue) {
      if (editValue.imageObj == "") {
        axios
          .put(
            `http://localhost:8080/images/updatename/${this.id}/${editValue.prodName}`
          )
          .then(() => {
        ...
        axios
          .put(
            `http://localhost:8080/images/update/${editValue.prodName}/${this.productByid.imageName}`,
            fd
          )
          .then(() => {
      ...
      axios
        .put(`http://localhost:8080/products/update/${this.id}`, editValue)
        .then((response) => {
    ...
    async fetchProductResult() {
      const res = await fetch(
        `http://localhost:8080/products/item/${this.id}`
      );
  ...
  async created() {
    ...
    this.imgURL = `http://localhost:8080/images/get/${this.productByid.imageName}`;
  },
};
</script>
