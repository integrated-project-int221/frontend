...
export default {
  ...
  data() {
    return {
      modalDelete: false,
      imageUrl: `http://172.22.0.3:3000/images/get/${this.product.imageName}`,
    };
  },
  methods: {
    checkImage() {
      try {
        this.imageUrl = `http://172.22.0.3:3000/images/get/${this.product.imageName}`;
      } catch (error) {
        this.imageUrl =
          "http://172.22.0.3:3000/images/get/OngYingGuitar.jpg";
        console.log(error);
      }
    ...
    deleteReview(id) {
      ...
      axios
        .delete(
          `http://172.22.0.3:3000/images/delete/${this.product.imageName}`
        )
        .then((response) => {
          return response.data;
        });
      axios
        .delete(`http://172.22.0.3:3000/products/delete/${id}`)
        .then((response) => {
          return response.data;
        })
    ...
    editArray(editValue) {
      ...
      axios
        .put(`http://172.22.0.3:3000/products/update/${id}`, editValue)
        .then((response) => {
