...
export default {
  ...
  methods: {
    ...
    async fetchColorsResult() {
      try {
        const res = await fetch("http://172.20.0.3:3000/colors");
    ...
    async fetchBrandsResult() {
      try {
        const res = await fetch("http://172.20.0.3:3000/brands");
        const data = await res.json();
        return data;
      } catch (error) {
        console.log(error);
    ...
    async fetchAllProductResult() {
      const res = await fetch("http://172.20.0.3:3000/products/items");
      const data = await res.json();
      return data;
    },
  },
